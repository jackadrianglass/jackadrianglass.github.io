...
{
	if (region.ao.O === region.ay.O)
	{
		return 'on line ' + region.ao.O;
	}
	return 'on lines ' + region.ao.O + ' through ' + region.ay.O;
}
...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { bh: 'hidden', a4: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { bh: 'mozHidden', a4: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { bh: 'msHidden', a4: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { bh: 'webkitHidden', a4: 'webkitvisibilitychange' }
		: { bh: 'hidden', a4: 'visibilitychange' };
}
...
var $author$project$TiledLines$generateDirections = function (settings) {
	var _v0 = A3($author$project$TiledLines$stepCounts, settings.bK, settings.aa, settings.W);
...
var $author$project$Theme$theme = {
	a0: A3($avh4$elm_color$Color$rgb255, 25, 23, 36),
	bd: A3($avh4$elm_color$Color$rgb255, 156, 207, 216),
	be: A3($avh4$elm_color$Color$rgb255, 246, 193, 119),
	bi: A3($avh4$elm_color$Color$rgb255, 82, 79, 103),
	bj: A3($avh4$elm_color$Color$rgb255, 33, 32, 46),
	bk: A3($avh4$elm_color$Color$rgb255, 64, 61, 82),
	bo: A3($avh4$elm_color$Color$rgb255, 196, 167, 231),
	bq: A3($avh4$elm_color$Color$rgb255, 235, 111, 146),
	bx: A3($avh4$elm_color$Color$rgb255, 110, 106, 134),
	bD: A3($avh4$elm_color$Color$rgb255, 38, 35, 58),
	bE: A3($avh4$elm_color$Color$rgb255, 49, 116, 143),
	bH: A3($avh4$elm_color$Color$rgb255, 235, 188, 186),
	bO: A3($avh4$elm_color$Color$rgb255, 144, 140, 170),
	bP: A3($avh4$elm_color$Color$rgb255, 31, 29, 46),
	bQ: A3($avh4$elm_color$Color$rgb255, 224, 222, 244)
};
var $author$project$Main$init = function (flags) {
	var drawingSettings = {a$: $author$project$Theme$theme.a0, W: flags.aq, bK: 20, bL: $author$project$Theme$theme.bi, aa: flags.ar - flags.B};
	var model = {
		s: {ae: $elm$core$Maybe$Nothing, am: drawingSettings},
		B: flags.B
	};
	return _Utils_Tuple2(
		model,
	...
	function (msg, model) {
		if (!msg.$) {
			...
			var newDrawing = _Utils_update(
				old,
				{
					ae: $elm$core$Maybe$Just(directions)
				});
			return _Utils_Tuple2(
				_Utils_update(
					model,
			...
			var oldSettings = model.s.am;
			var oldDrawingModel = model.s;
			var newSettings = _Utils_update(
				oldSettings,
				{W: dimensions.W, aa: dimensions.aa - model.B});
			var newDrawingModel = _Utils_update(
				oldDrawingModel,
				{ae: $elm$core$Maybe$Nothing, am: newSettings});
...
var $1602$elm_feather$FeatherIcons$defaultAttributes = function (name) {
	return {
		V: $elm$core$Maybe$Just('feather feather-' + name),
		Y: 24,
		P: '',
		Z: 2,
		_: '0 0 24 24'
...
var $1602$elm_feather$FeatherIcons$toHtml = F2(
	function (attributes, _v0) {
		var src = _v0.h;
		var attrs = _v0.f;
		var strSize = $elm$core$String$fromFloat(attrs.Y);
		var baseAttributes = _List_fromArray(
			[
				...
				$elm$core$String$fromFloat(attrs.Z)),
				$elm$svg$Svg$Attributes$viewBox(attrs._)
			]);
...
var $author$project$TiledLines$drawing = F2(
	function (settings, directions) {
		var stepSize = $elm$core$Basics$floor(settings.bK);
		var shapeSettings = _List_fromArray(
			[
				$joakin$elm_canvas$Canvas$Settings$stroke(settings.bL),
				$joakin$elm_canvas$Canvas$Settings$fill(settings.a$),
...
var $joakin$elm_canvas$Canvas$renderTextFill = F5(
	function (txt, x, y, maybeColor, cmds) {
		return A2(
			$elm$core$List$cons,
			A4($joakin$elm_canvas$Canvas$Internal$CustomElementJsonApi$fillText, txt.bQ, x, y, txt.ah),
			function () {
				if (!maybeColor.$) {
					var color = maybeColor.a;
					return A2(
...
var $joakin$elm_canvas$Canvas$renderTextStroke = F5(
	function (txt, x, y, maybeColor, cmds) {
		return A2(
			$elm$core$List$cons,
			A4($joakin$elm_canvas$Canvas$Internal$CustomElementJsonApi$strokeText, txt.bQ, x, y, txt.ah),
			function () {
				if (!maybeColor.$) {
					var color = maybeColor.a;
					return A2(
	...
	function (x, y, t, cmds) {
		return A2(
			...
			function () {
				if (!t.$) {
					var image = t.a;
					return A9($joakin$elm_canvas$Canvas$Internal$CustomElementJsonApi$drawImage, 0, 0, image.aa, image.W, x, y, image.aa, image.W, image.X);
				} else {
					var sprite = t.a;
					var image = t.b;
					return A9($joakin$elm_canvas$Canvas$Internal$CustomElementJsonApi$drawImage, sprite.aX, sprite.aY, sprite.aa, sprite.W, x, y, sprite.aa, sprite.W, image.X);
				}
			}(),
			cmds);
	});
				...
				function (tagName, width, height) {
					return (tagName === 'IMG') ? $elm$core$Maybe$Just(
						$joakin$elm_canvas$Canvas$Internal$Texture$TImage(
							{W: height, X: image, aa: width})) : $elm$core$Maybe$Nothing;
				}),
	...
	function (options, attrs, entities) {
		return A3(
			...
			A2(
				...
				A2(
					$elm$core$List$cons,
					$elm$html$Html$Attributes$height(options.W),
					A2(
						$elm$core$List$cons,
...
var $author$project$TiledLines$view = function (model) {
	var canvasElements = function () {
		...
		if (_v0.$ === 1) {
			...
			return A2($author$project$TiledLines$drawing, model.am, directions);
		}
	}();
	return A3(
		$joakin$elm_canvas$Canvas$toHtml,
		_Utils_Tuple2(
			$elm$core$Basics$round(model.am.aa),
			$elm$core$Basics$round(model.am.W)),
		_List_Nil,
		canvasElements);
};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$author$project$TiledLines$view(model.s)
					])),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
					...
					return A2(
						...
						function (scrollBarWidth) {
							return $elm$json$Json$Decode$succeed(
								{B: scrollBarWidth, aq: windowHeight, ar: windowWidth});
						},
